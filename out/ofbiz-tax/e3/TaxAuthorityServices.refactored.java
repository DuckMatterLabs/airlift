...
public class TaxAuthorityServices {
    ...
    public static Map<String, Object> rateProductTaxCalcForDisplay(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            if ("Y".equals(productStore.getString("showPricesWithVatTax"))) {
                Set<GenericValue> taxAuthoritySet = resolveVatTaxAuthorities(delegator, productStore);
    ...
    private static List<GenericValue> getTaxAdjustments(Delegator delegator, GenericValue product,
        ...
        // AIRLIFT: F-TAS-031
        // store expr
        EntityCondition storeCond = buildProductStoreCondition(productStore);

        // AIRLIFT: F-TAS-032
        // build the TaxAuthority expressions (taxAuthGeoId, taxAuthPartyId)
        EntityCondition taxAuthoritiesCond = buildTaxAuthoritiesCondition(taxAuthoritySet);

        try {
            EntityCondition productCategoryCond = setProductCategoryCond(delegator, product);
            ...
            EntityCondition mainCondition = buildRateRuleLookupCondition(storeCond, taxAuthoritiesCond,
                    productCategoryCond, itemPrice, itemAmount);

            // finally ready... do the rate query
            List<GenericValue> rateRuleList = EntityQuery.use(delegator).from("TaxAuthorityRateProduct")
                    .where(mainCondition).orderBy("minItemPrice", "minPurchase", "fromDate").filterByDate().queryList();

            if (rateRuleList.isEmpty()) {
            ...
            for (GenericValue taxAuthorityRateProduct : rateRuleList) {
                adjustments.addAll(buildTaxAdjustmentsForRateRule(delegator, taxAuthorityRateProduct, product,
                        productStore, payToPartyId, billToPartyId, itemPrice, itemQuantity, itemAmount,
                        shippingAmount, orderPromotionsAmount, weight, nowTimestamp));
            }
        } catch (GenericEntityException e) {
    ...
    // AIRLIFT: F-TAS-031
    // store expr: match rate rules scoped to this ProductStore or store-agnostic (null) rules
    private static EntityCondition buildProductStoreCondition(GenericValue productStore) {
        if (productStore != null) {
        ...
        return EntityCondition.makeCondition("productStoreId", EntityOperator.EQUALS, null);
    }

    // AIRLIFT: F-TAS-032
    // build the TaxAuthority expressions (taxAuthGeoId, taxAuthPartyId), always including the _NA_ authority
    private static EntityCondition buildTaxAuthoritiesCondition(Set<GenericValue> taxAuthoritySet) {
        ...
        return EntityCondition.makeCondition(taxAuthCondOrList, EntityOperator.OR);
    }

    // AIRLIFT: F-TAS-036
    // combine store, authority and category predicates with the minimum-item-price and
    // minimum-purchase threshold rules (a null threshold means "no threshold")
    private static EntityCondition buildRateRuleLookupCondition(EntityCondition storeCond,
            EntityCondition taxAuthoritiesCond, EntityCondition productCategoryCond, BigDecimal itemPrice,
        ...
        return EntityCondition.makeCondition(mainExprs, EntityOperator.AND);
    }

    // AIRLIFT: F-TAS-007
    // resolve the VAT tax authorities configured on the store for showing prices with VAT
    private static Set<GenericValue> resolveVatTaxAuthorities(Delegator delegator, GenericValue productStore)
        ...
        return taxAuthoritySet;
    }

    // AIRLIFT: F-TAS-038
    // apply one matched rate rule: figure the taxable base, compute the tax, and build the
    // resulting OrderAdjustment(s) (main tax, optional negative VAT-discount, optional VAT price correction)
    private static List<GenericValue> buildTaxAdjustmentsForRateRule(Delegator delegator,
            GenericValue taxAuthorityRateProduct, GenericValue product, GenericValue productStore,
            String payToPartyId, String billToPartyId, BigDecimal itemPrice, BigDecimal itemQuantity,
            BigDecimal itemAmount, BigDecimal shippingAmount, BigDecimal orderPromotionsAmount,
            BigDecimal weight, Timestamp nowTimestamp) throws GenericEntityException {
        List<GenericValue> ruleAdjustments = new LinkedList<>();
        ...
        BigDecimal taxable = computeTaxableAmount(product, taxAuthorityRateProduct, itemAmount, shippingAmount,
        ...
        String taxAuthGlAccountId = resolveTaxAuthGlAccountId(delegator, taxAuthPartyId, taxAuthGeoId, payToPartyId);
        ...
        if (product != null && taxAuthPartyId != null && taxAuthGeoId != null) {
            ...
            productPrice = resolveTaxProductPrice(delegator, product, productStore, taxAuthGeoId, taxAuthPartyId);
        }

        // AIRLIFT: F-TAS-042
        GenericValue taxAdjValue = delegator.makeValue("OrderAdjustment");
        BigDecimal discountedSalesTax = applyVatInPriceAdjustment(taxAdjValue, productPrice, itemPrice,
                itemQuantity, itemAmount, taxRate, taxAmount);
        ...
        if (UtilValidate.isNotEmpty(billToPartyId) && UtilValidate.isNotEmpty(taxAuthGeoId)) {
            applyPartyTaxExemptions(delegator, taxAdjValue, billToPartyId, taxAuthGeoId, taxAuthPartyId, taxAmount,
                    nowTimestamp);
        } else {
        ...
        if (discountedSalesTax.compareTo(BigDecimal.ZERO) < 0) {
            ...
            ruleAdjustments.add(taxAdjValueNegative);
        }
        ruleAdjustments.add(taxAdjValue);

        // AIRLIFT: F-TAS-046
        GenericValue correctionAdjValue = buildVatPriceCorrection(delegator, taxAuthorityRateProduct, productPrice,
                itemQuantity, taxRate, taxAuthGeoId, taxAuthPartyId, taxAuthGlAccountId);
        if (correctionAdjValue != null) {
            ruleAdjustments.add(correctionAdjValue);
        }
    ...
    // AIRLIFT: F-TAS-038
    // sum the taxable base: item amount when the product is taxable, plus shipping and/or
    // promotion amounts when the rate rule says those are taxable (a null flag means "taxable")
    private static BigDecimal computeTaxableAmount(GenericValue product, GenericValue taxAuthorityRateProduct,
            BigDecimal itemAmount, BigDecimal shippingAmount, BigDecimal orderPromotionsAmount) {
    ...
    // get glAccountId from TaxAuthorityGlAccount entity using the payToPartyId as
    // the organizationPartyId
    private static String resolveTaxAuthGlAccountId(Delegator delegator, String taxAuthPartyId, String taxAuthGeoId,
            String payToPartyId) throws GenericEntityException {
        ...
        if (taxAuthorityGlAccount != null) {
            return taxAuthorityGlAccount.getString("glAccountId");
        }
    ...
    // AIRLIFT: F-TAS-041
    // find a ProductPrice for the productId and taxAuth* values, falling back to the parent
    // (virtual) product when the variant itself has none
    private static GenericValue resolveTaxProductPrice(Delegator delegator, GenericValue product,
            GenericValue productStore, String taxAuthGeoId, String taxAuthPartyId) throws GenericEntityException {
        GenericValue productPrice = getProductPrice(delegator, product, productStore, taxAuthGeoId, taxAuthPartyId);
    ...
    // AIRLIFT: F-TAS-042
    // when the price already includes VAT, record the included tax and the discount-adjusted
    // sales tax; otherwise set the plain sales-tax amount. Returns the (negative or zero)
    // discountedSalesTax used to build a corrective adjustment.
    private static BigDecimal applyVatInPriceAdjustment(GenericValue taxAdjValue, GenericValue productPrice,
            BigDecimal itemPrice, BigDecimal itemQuantity, BigDecimal itemAmount, BigDecimal taxRate,
        ...
        return discountedSalesTax;
    }

    // AIRLIFT: F-TAS-046
    // when the tax is NOT already in the price but a price-with-tax was entered, build a
    // VAT_PRICE_CORRECT adjustment for the difference between the entered and calculated totals
    private static GenericValue buildVatPriceCorrection(Delegator delegator, GenericValue taxAuthorityRateProduct,
            GenericValue productPrice, BigDecimal itemQuantity, BigDecimal taxRate, String taxAuthGeoId,
            String taxAuthPartyId, String taxAuthGlAccountId) {
    ...
    private static void applyPartyTaxExemptions(Delegator delegator, GenericValue taxAdjValue, String billToPartyId,
            String taxAuthGeoId, String taxAuthPartyId, BigDecimal taxAmount, Timestamp nowTimestamp)
            throws GenericEntityException {

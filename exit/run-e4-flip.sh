...
set +e
"$TARGET_DIR/harness/run-tests.sh" airliftblind > "$AIRLIFT_ROOT/runs/e4.log" 2>&1
RUN_EXIT=$?
"$PY" "$TARGET_DIR/harness/report-claims.py" \
  --results "$TEST_RESULTS_DIR/airliftblind.xml" \
  --tests-dir "$BLIND_TEST_DIR" --ir "$OUT_DIR/ir" | tee "$AIRLIFT_ROOT/runs/e4-spine.txt"
SPINE_EXIT=$?
set -e
"$PY" "$MUT" revert
...
if ! grep -q "✗ TAX\.EXEMPT" "$AIRLIFT_ROOT/runs/e4-spine.txt"; then
...
grep -E '^\s+✗' "$AIRLIFT_ROOT/runs/e4-spine.txt"
